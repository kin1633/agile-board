...
test('アクティブスプリントのタスクのみドロップダウンに含まれる', function () {
    ...
    $taskInActive = Issue::factory()->for($repo)->create(['parent_issue_id' => $parentIssue->id, 'sprint_id' => $activeSprint->id, 'state' => 'open']);
    $taskInClosed = Issue::factory()->for($repo)->create(['parent_issue_id' => $parentIssue->id, 'sprint_id' => $closedSprint->id, 'state' => 'open']);
...
test('クローズ済みタスクはドロップダウンに含まれない', function () {
    $user = User::factory()->create();
    $repo = Repository::factory()->create();
    $activeSprint = Sprint::factory()->create(['state' => 'open']);

    $parentIssue = Issue::factory()->for($repo)->create(['parent_issue_id' => null]);
    $openTask = Issue::factory()->for($repo)->create(['parent_issue_id' => $parentIssue->id, 'sprint_id' => $activeSprint->id, 'state' => 'open']);
    $closedTask = Issue::factory()->for($repo)->create(['parent_issue_id' => $parentIssue->id, 'sprint_id' => $activeSprint->id, 'state' => 'closed']);
