...
export default function HolidaysSettings({ holidays, year }: Props) {
    ...
    const handleYearChange = (newYear: number) => {
        ...
        router.get(settings.holidays().url, { year: newYear }, { preserveState: true });
    };

    const handleImport = () => {
        importForm.setData('year', selectedYear);
        importForm.post(settingsHolidays.import().url, { preserveScroll: true });
    };
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {importForm.errors.year && (
                        <p className="text-sm text-destructive">{importForm.errors.year}</p>
                    )}
                </div>
                ...
                <div className="rounded-xl border border-sidebar-border/70 bg-card">
                    {holidays.length > 0 ? (
                        ...
                        <p className="px-6 py-4 text-sm text-muted-foreground">
                            {selectedYear}年の休日はまだ登録されていません。「祝日を取得」ボタンでインポートしてください。
                        </p>
                    )}
                </div>
                ...
                <div className="rounded-xl border border-sidebar-border/70 bg-card px-6 py-4">
                    <h2 className="mb-3 text-sm font-medium">現場休日を追加</h2>
                    <form onSubmit={handleAdd} className="flex items-end gap-3">
                        <div className="flex flex-col gap-1">
                            ...
                            />
                            {addForm.errors.date && (
                                <p className="text-xs text-destructive">{addForm.errors.date}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-1">
