...
export default function MilestoneEdit({ milestone }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: '編集', href: milestoneRoutes.edit({ milestone: milestone.id }).url },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(milestoneRoutes.update({ milestone: milestone.id }).url);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${milestone.title} - 編集`} />
            <div className="flex flex-col gap-6 p-6">
                <h1 className="text-xl font-semibold">マイルストーン編集</h1>
